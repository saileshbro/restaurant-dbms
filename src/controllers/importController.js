...
module.exports.getImportCompanies = async (req, res) => {
  try {
    const results = await pool.query(
      `SELECT import_company_id,name,phone,email,address,total_transactions,remain_transactions,purchase_type FROM import_company INNER JOIN contact_info ON import_company.import_company_id= contact_info.contact_info_id ORDER BY name`
    );
    if (!results) {
...
module.exports.getImportCompany = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT import_company_id,name,phone,email,address,total_transactions,remain_transactions,purchase_type FROM import_company INNER JOIN contact_info ON import_company.import_company_id= contact_info.contact_info_id where import_company_id=? ORDER BY name`,
      [import_company_id]
    );
    if (!result) {
    ...
    if (insertImport.affectedRows == 1) {
      import_details.forEach(item => {
        const insertImportDetails = pool.query(
          "INSERT INTO import_detail SET import_good=?,bill_no=?,quantity=?,price=?",
          [item.import_good, bill_no, item.quantity, parseFloat(item.price)]
        );
