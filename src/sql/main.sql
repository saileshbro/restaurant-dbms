...
(10),
    FOREIGN KEY
(food_category_id) REFERENCES food_category
(food_category_id) ON
DELETE CASCADE
);
...
(100),
    ...
    PRIMARY KEY
(menu_name,menu_start_date)
);

CREATE TABLE
IF NOT EXISTS menu_content
(
    menu_name VARCHAR
(100),
    food_item_name VARCHAR
(100),
    ...
    FOREIGN KEY
(food_item_name) REFERENCES food_item
(food_item_name) ON
DELETE CASCADE,
...
(11) PRIMARY KEY,
    foreign KEY
(customer_id) REFERENCES contact_info
(contact_info_id)
);
CREATE TABLE
...
(11) PRIMARY KEY,
    ...
    FOREIGN KEY
(staff_id) REFERENCES contact_info
(contact_info_id),
    FOREIGN KEY
(staff_category) REFERENCES staff_category
(staff_category) ON
DELETE CASCADE);

CREATE TABLE
IF NOT EXISTS import_company
(
    import_company_id INT
(11) PRIMARY KEY,
    total_transaction double
(10,2) DEFAULT 0,
    remain_transaction double
(10,2) DEFAULT 0,
    purchase_type VARCHAR
(128),
    FOREIGN KEY
(import_company_id) REFERENCES contact_info
(contact_info_id)
);
...
(11),
    foreign KEY
(restaurant_id) REFERENCES contact_info
(contact_info_id)
);


CREATE TABLE
IF NOT EXISTS restaurant_table
(
    table_no INT
(11) PRIMARY KEY,
...
(bill_no,import_date),
    FOREIGN KEY
(import_company_id) REFERENCES import_company
(import_company_id)
);
...
(customer_id),
    FOREIGN KEY
(table_no) REFERENCES restaurant_table
(table_no)
);
CREATE TABLE
IF NOT EXISTS bill
