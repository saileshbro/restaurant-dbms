...
(
    ...
    is_available bit,
    FOREIGN KEY
                (food_item_id) REFERENCES food_item
                (food_item_id) ON
                DELETE CASCADE,
    FOREIGN KEY(menu_id)
                REFERENCES menu
    ...
    phone varchar
                (30),
    foreign key
                (resturant_id) references restaurant
                (resturant_id) on
                DELETE CASCADE,
    foreign key (import_company_id)
                references import_company
                (import_company_id) on
                DELETE CASCADE,
   ...
   staff_id int
    ...
    joined_date date,
    FOREIGN key
                (staff_category_id) references staff_cat
                (staff_category_id) on
                DELETE CASCADE);
    ...
    FOREIGN KEY customer_id REFERENCES customer
                (customer_id),
    FOREIGN KEY table_id REFERENCES restaurant_table
                (table_id)
);
